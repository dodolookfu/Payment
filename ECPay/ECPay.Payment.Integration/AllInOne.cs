...
namespace ECPay.Payment.Integration
{
    ...
    public class AllInOne : AllInOneMetadata, IDisposable
    {
        ...
        {            
            ...
            if (errList.Count == 0)
            {
                ...
                if (ServiceMethod == HttpMethod.HttpPOST) {
                    SortedDictionary<string, string> postValue = new SortedDictionary<string, string>();
                    string[] val = new string[2]; //參數key value
                    var param = szParameters.Substring(1).Split('&');
                    foreach (var item in param) {
                        val = item.Split('=');
                        postValue.Add(val[0], val[1]);                        
                    }
                    FormPost action = new FormPost(this.ServiceURL, postValue);
                    action.Post();
                } else {
        ...
        private string ServerPost(string parameters)
        {
            ...
            WebRequest webRequest = WebRequest.Create(this.ServiceURL); //(HttpWebRequest)
            {
                webRequest.ContentType = "application/x-www-form-urlencoded";
                webRequest.Method = "POST";
                webRequest.ContentLength = byContent.Length;
                //webRequest.Referer = "https://payment-stage.ecpay.com.tw";
